...
export class CurrencyConverter implements Converter<Currency, CurrencyDto> {
  ...
  toModel(model: CurrencyDto): Promise<Currency> {
    return Promise.resolve(new Currency(
      model.id, model.name, model.symbol, model.decimalPoints,
      Object.keys(model.exchangeRates).map(Number).map(t => (t as keyof typeof model.exchangeRates)).map(key => {
        return {
  ...
  toDTO(dto: Currency): CurrencyDto {
    return CurrencyDto.create({
      ...
      exchangeRates: Object.keys(dto.exchangeRates).map(Number).map(t => (t as keyof typeof dto.exchangeRates)).map(key => {
        return {
