...
import {
  ...
  differenceInMonths,
  differenceInWeeks,
  differenceInYears, isBefore, isSameDay,
    ...
    while (!isSameDay(upTo, today)) {
      ...
      while (i >= 0 && (isBefore(augmentedTransactions[i].date, upTo) || isSameDay(today, upTo))) {
        if (augmentedTransactions[i].sender?.isMine ?? false) {
          data[data.length - 1].Total -= augmentedTransactions[i].amount / Math.pow(10, augmentedTransactions[i].currency.decimalPoints)
        }
        if (augmentedTransactions[i].receiver?.isMine ?? false) {
          data[data.length - 1].Total += augmentedTransactions[i].receiverAmount / Math.pow(10, augmentedTransactions[i].receiverCurrency.decimalPoints)
        }
        i -= 1
      }
    }
    ...
    return (
      <div style={{height: "30rem"}}>
        <ResponsiveStream
          ...
          axisBottom={{
            format: i => i % showLabelEveryFactor === 0 ? keys[i].toDateString() : "",
            tickRotation: -45,
          }}
          curve="linear"
          ...
          stackTooltip={tooltipProps => (
            <IonCard style={{padding: "0.5rem"}}>
              <div>
                {keys[tooltipProps.slice.index].toDateString()}
              </div>
              {tooltipProps.slice.stack.map(s => (
                <div key={`line-chart-overlay-${keys[tooltipProps.slice.index].toDateString()}-${s.layerLabel}`}
