...
import { CreateCategoryRequest, GetAllCategoriesRequest } from "../messaging/dto/category"
...
export default class CategoryRepository {
  ...
  async create(name: string, iconName: string, parentId: number, iconBackground: string, iconColor: string): Promise<Category> {
    const response = await this.client.createCategory(CreateCategoryRequest.create({
      name, iconName, parentId, iconBackground, iconColor,
    })).response
    return new Category(response.id, name, iconName, iconColor, iconBackground, parentId)
  }
}
